...
use crate::ecs::component::LocalUserSpawn;
...
pub fn send_packet_message<'a, T>(message: EcsMessage, user_spawns: T)
where
    T: shipyard::Get<Out = &'a LocalUserSpawn>,
{
    if let Some(connection_id) = message.connection_id() {
        if let Ok(spawn) = user_spawns.try_get(connection_id) {
            send_message(message, &spawn.channel);
        } else {
